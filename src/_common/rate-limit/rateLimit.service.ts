...
export class RateLimitService {
	...
	constructor(
		...
		this.DEFAULT_TTL_MS = parseInt(
			this.configService.get<string>(RATE_LIMIT_DEFAULT_TTL_MS, "60000"),
		);
		this.DEFAULT_LIMIT = parseInt(
			this.configService.get<string>(RATE_LIMIT_DEFAULT_COUNT, "100"),
		);
	...
	private cleanup(): void {
		const now = Date.now();
		this.logger.log(`Start clean up. before : ${this.store.size}`, {
		...
		this.logger.log(`End clean up. after : ${this.store.size}`, {
			className: "RateLimitService",
		});
	}
}
