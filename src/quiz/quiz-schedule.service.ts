    ...
    try {
      ...
      for (let i = 0; i < tryCount; i++) {
        ...
        if (this.deleteContext(deleteID)) {
          throw new Error(`Can't delete Context during updating fixed Context. ID : ${deleteID}`);
        }
      }
      //re-arrange
  ...
  */
  ...
  private addContexts(contexts: QuizContext[], isFixed: boolean = false): boolean {
    ...
    try {
      ...
      this.logger.log(
        `[${QuizScheduleService.name}] addContext finish` + `${JSON.stringify(tasks, null, 2)}`,
        {
          className: QuizScheduleService.name,
        },
      );
  ...
  private sortByLowPriority(hashNodes: ContextHashNode[]): ContextHashNode[] {
    ...
    return temp.sort((a, b) => {
      if (b.scheduleCount === a.scheduleCount) {
        //나중에 선택될 Context 제거
        return (
          this.getSchedulingOffset(b.schedulerIndex) - this.getSchedulingOffset(a.schedulerIndex)
        );
      }
