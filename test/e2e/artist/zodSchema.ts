...
export const zShowArtist = z.object({
	...
	birth_date: z.date().nullable(), // API 응답이 문자열이면 z.string().datetime().nullable()
	death_date: z.date().nullable(), // 동일하게 z.string().datetime().nullable()
	info_url: z.string().nullable(),
});

// ShowArtistResponse 클래스에 대응되는 스키마
export const ShowArtistResponse = zShowArtist.extend({
