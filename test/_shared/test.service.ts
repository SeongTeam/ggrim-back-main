...
import { factoryQuizReaction, QuizDislikeDummy, QuizLikeDummy } from "./stub/quiz-reaction.stub";
...
export class TestService implements OnModuleInit {
	...
	async initTables() {
		...
		try {
			...
			for (const table of tables) {
				await qr.query(
					`ALTER TABLE core.${table} ALTER COLUMN id SET GENERATED BY DEFAULT`,
				);
			}
	...
	async insertPaintingStubs(paintingStubs: ReadonlyDeep<InsertPaintingArgs[]>) {
		...
		try {
			const relationMap: Map<number, PaintingRelations> = new Map();
	...
	async insertOneChoiceQuizStubs(quizStubs: ReadonlyDeep<InsertOneChoiceQuizzesArgs[]>) {
		const extractRelations = (stub: InsertOneChoiceQuizzesArgs) => {
			const { answer, distractors, user } = stub;
			...
			return { user, answer, distractors, tags, styles, artists };
		};
		...
		const relationMap: Map<number, OneChoiceQuizzesRelations> = new Map();
		...
		try {
			...
			quizWithRelations = await manager
				...
				.leftJoinAndSelect("quiz.user", "quiz_user")
				.getMany();
		} catch (error) {
	...
	async seedOneChoiceQuizzes(
		...
		const seedQuizArgs: InsertOneChoiceQuizzesArgs[] = Array(count)
			...
			.map(() => {
				...
				const user = getRandomElement(owners);
