...
export class AddQuizLikeTableAndDislikeTable1745399514288 implements MigrationInterface {
  name = 'AddQuizLikeTableAndDislikeTable1745399514288';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE "quiz_like" ("created_date" TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "updated_date" TIMESTAMP(6) WITH TIME ZONE DEFAULT now(), "deleted_date" TIMESTAMP(6) WITH TIME ZONE, "version" integer, "id" SERIAL NOT NULL, "user_id" uuid NOT NULL, "quiz_id" uuid NOT NULL, CONSTRAINT "UQ_8da3653df7f06fcfa481ed5a2ff" UNIQUE ("user_id", "quiz_id"), CONSTRAINT "PK_79e0a89273bcd881c1dfa854a73" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE TABLE "quiz_dislike" ("created_date" TIMESTAMP(6) WITH TIME ZONE NOT NULL DEFAULT ('now'::text)::timestamp(6) with time zone, "updated_date" TIMESTAMP(6) WITH TIME ZONE DEFAULT now(), "deleted_date" TIMESTAMP(6) WITH TIME ZONE, "version" integer, "id" SERIAL NOT NULL, "user_id" uuid NOT NULL, "quiz_id" uuid NOT NULL, CONSTRAINT "UQ_11a3a7152b5e23a0ec3f67983d6" UNIQUE ("user_id", "quiz_id"), CONSTRAINT "PK_54f98d5f852e75d52e8a51825ed" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
